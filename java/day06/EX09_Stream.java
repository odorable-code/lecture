...
public class EX09_Stream {
    public static void main(String[] args) {
         ...
         *   - 중간 처리 메서드와 최종 처리 메서드를 제공해서 여러 연산을 한번에 할 수 있음
         *
         * - filter
         *    - 중간 처리 메서드
         *    - 객체를 중 원하는 객체들만 고를 때 사용
         *
         * - map
         *   - 중간 처리 메서드
        ...
        list.add(new Student(1, 3, 1, "나길동"));
        list.add(new Student(2, 2, 1, "임꺽정"));
        list.add(new Student(3, 1, 1, "허준"));
        ...
        // 등록된 학생들의 평균 학년
        stream = list.stream();
        double avg = stream.mapToInt(Student::getGrade)// mapToInt(s -> s.getGrade())와 같음
                           .average()
                           .orElse(0); // 평균을 낼 수 없으면 0
