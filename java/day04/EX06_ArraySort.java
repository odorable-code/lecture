...
public class EX06_ArraySort {
    public static void main(String[] args) {
        ...
        int num1 = 10, num2 = 20;
        // num1에 num2의 값 20이 저장되고, num2에 num1의 값 10이 저장되도록 작성
        System.out.println("num1 = " + num1);
        System.out.println("num2 = " + num2);

        num1 = num2; // num2의 값이 num1의 값을 덮어쓰기 해서 num1의 값이 사라짐
        num2 = num1; // 이후에 20을 다시 num2 에 해도 10은 돌아오지 않음
         ...
         * 5 7 9 3 4 6 8 10 2 1  : 두 사이클 후 결과, 1과 2가 정렬
         * */
        int arr[] = {1, 3, 5, 7, 9, 2, 4, 6, 8, 10};
        for (int i = 0; i < arr.length-1; i++) {
            for (int j = 0; j < arr.length-1; j++) {
                // 내림차 순으로 앞에 수가 뒤 수 보다 작으면 교환
                if (arr[j] < arr[j+1]) {
                    int tmp1 = arr[j];
                    arr[j] = arr[j+1];
                    arr[j+1] = tmp1;
